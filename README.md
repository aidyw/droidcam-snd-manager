...
***UPDATE as of upgrade to Ubuntu 22.04.2
The .scope units generated by Gnome have changed name. What was formally:
```
gnome-launched-droidcam.desktop-<PID>.scope
```
has now become:
```
app-gnome-droidcam-<PID>.scope 
```
Therefore the wildcard drop-in directory must be appropriatly named:
```
app-gnome-droidcam-.scope.d 
````
***
...
It is possible to specifically specify the ALSA index number to use for DroidCam or ‘any’ to allow the module to be inserted at the next available index. This is done in the script called by the droidcam.desktop file:
```
